...
class GenerateMypy(GenerateCommandExtension):  # type: ignore
    def generate(
        self,
        package_name: str,
        ...
        package_share_path = pathlib.Path(
            get_package_share_directory("rosidl_generator_mypy")
        )
        templates_path = package_share_path / "resource"

        # Normalize interface definition format to .idl
        idl_interface_files: List[str] = []
        non_idl_interface_files: List[str] = []
